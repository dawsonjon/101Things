...
int main() {
  ...
  uint32_t frequency_shift_steps = static_cast<uint32_t>(4294967296.0 * frequency_shift_Hz/audio_sample_rate_Hz);
  ...
  while (true) {
    ...
    for(uint16_t idx=0; idx<1024; ++idx)
    {
      ...
      uint32_t phase_MSB = downshift_phase >> 22; //keep 10 MSBs of phase 32-10 = 22
      ...
      int16_t i_shifted = (((int32_t)i * rotation_i) - ((int32_t)q * rotation_q)) >> 15;
      int16_t q_shifted = (((int32_t)q * rotation_i) + ((int32_t)i * rotation_q)) >> 15;
      ...
      const int16_t i_new = (((int32_t)i_shifted * rotation_i) - ((int32_t)q_shifted * rotation_q)) >> 15;
