...
void eq::process_sample(int32_t & sample)
{
  ...
  if(pointer == 32u)
  {
    ...
    //input has 16 bits.
    //ADC has enob of 9.5 bits and 16x oversampling adds 2 bits giving about 12 bits containing useful information
    //lower 4 bits are just noise, and can be discarded without degrading SNR.
    //This is achieved by scaling the window by 1/16 and avoids the need for another multiplication.
    ...
    //The 64 point FFT has a growth of 6 bits, the least significant 3 bits contain only noise and are removed inside the FFT
    //This leaves us with 18 bits of data, 15 of which are useful.
    ...
    for(uint16_t i=0; i<33; i++){
      buffer[ping_pong][i] = eq_scale(buffer[ping_pong][i], frequency_response[i]);
      buffer_imag[i] = eq_scale(buffer_imag[i], frequency_response[i]);
    }
    for(uint16_t i=33; i<64; i++){
      buffer[ping_pong][i] = eq_scale(buffer[ping_pong][i], frequency_response[64-i]);
