...
// filename: guitar_effects_ui.cpp
// description:
// License: MIT
//
...
float dB2lin(float x) { return powf(10.0f, x / 20.0f); }

const int32_t eq_gains[] = {
    eq_const(dB2lin(-36)), eq_const(dB2lin(-33)), eq_const(dB2lin(-30)),
    eq_const(dB2lin(-27)), eq_const(dB2lin(-24)), eq_const(dB2lin(-21)),
    eq_const(dB2lin(-18)), eq_const(dB2lin(-15)), eq_const(dB2lin(-12))};

const FixedPoint gains[] = {
    FixedPoint::from_float(dB2lin(0)),  FixedPoint::from_float(dB2lin(3)),
    FixedPoint::from_float(dB2lin(6)),  FixedPoint::from_float(dB2lin(9)),
    FixedPoint::from_float(dB2lin(12)), FixedPoint::from_float(dB2lin(15)),
    FixedPoint::from_float(dB2lin(18)), FixedPoint::from_float(dB2lin(21)),
...
menu_item *eq_menu_items[] = {&eq_band1, &eq_band2, &eq_band3, &eq_band4,
                              &eq_band5};
menu eq_menu("EQ", "Band 1#Band 2#Band 3#Band 4#Band 5", eq_menu_items, 4);

// Distorion Menus
enum_entry
    distortion_effect("Off#Cubic#Quadratic#Full Wave#Half Wave#Foldback#Fuzz",
                      0, 6);
...
menu_item *distortion_menu_items[] = {&distortion_effect, &distortion_gain,
                                      &distortion_offset};
menu distortion_menu("Distortion", "Effect#Gain#Offset", distortion_menu_items,
                     2);
...
menu_item *flanger_menu_items[] = {&flanger_depth_ms, &flanger_rate_Hz,
                                   &flanger_delay_ms, &flanger_feedback};
menu flanger_menu("Flanger", "Depth (ms)#Rate (Hz)#Delay (ms)#Feedback",
                  flanger_menu_items, 3);
...
menu_item *chorus_menu_items[] = {
    &chorus1_depth_ms, &chorus1_rate_Hz, &chorus1_delay_ms, &chorus1_feedback,
...
menu chorus_menu("Chorus",
                 "Depth 1 (ms)#Rate 1 (ms)#Delay 1 (ms)#Feedback 1#Depth 2 "
                 "(ms)#Rate 2 (ms)#Delay 2 (ms)#Feedback 2#Depth 3 (ms)#Rate 3 "
                 "(ms)#Delay 3 (ms)#Feedback 3",
                 chorus_menu_items, 11);
...
menu_item *modulator_items[] = {&modulator_effect, &flanger_menu, &chorus_menu,
                                &tremolo_menu,     &vibrato_menu, &phaser_menu};
menu modulator_menu("Modulator",
                    "Effect#Flanger Settings#Chorus Settings#Tremolo "
                    "Settings#Vibrato Settings#Phaser Settings",
                    modulator_items, 5);

// main menu
menu_item *menu_items[] = {&patch,           &preamp_menu, &eq_menu,
                           &distortion_menu, &delay_menu,  &modulator_menu};
menu main_menu("Menu", "Patch#Preamp#EQ#Distorion#Delay#Modulator", menu_items,
               5);
...
void guitar_effects_ui::do_ui(effects &e, s_effect &settings) {
  ...
  if (show_menu) {
    ...
    settings.distortion_effect =
        static_cast<e_distortion_effect>(distortion_effect.m_value);
    settings.distortion_offset =
        FixedPoint::from_float(distortion_offset.m_value);
    ...
    settings.modulator_effect =
        static_cast<e_modulator_effect>(modulator_effect.m_value);
    settings.flanger_depth_ms =
        FixedPoint::from_float(flanger_depth_ms.m_value);
    settings.flanger_rate_steps =
        frequency_Hz_to_steps(flanger_rate_Hz.m_value);
    settings.flanger_delay_ms = flanger_delay_ms.m_value;
    settings.flanger_feedback =
        FixedPoint::from_float(flanger_feedback.m_value);
    settings.chorus1_depth_ms =
        FixedPoint::from_float(chorus1_depth_ms.m_value);
    settings.chorus1_rate_steps =
        frequency_Hz_to_steps(chorus1_rate_Hz.m_value);
    settings.chorus1_delay_ms = chorus1_delay_ms.m_value;
    settings.chorus1_feedback =
        FixedPoint::from_float(chorus1_feedback.m_value);
    settings.chorus2_depth_ms =
        FixedPoint::from_float(chorus2_depth_ms.m_value);
    settings.chorus2_rate_steps =
        frequency_Hz_to_steps(chorus2_rate_Hz.m_value);
    settings.chorus2_delay_ms = chorus2_delay_ms.m_value;
    settings.chorus2_feedback =
        FixedPoint::from_float(chorus2_feedback.m_value);
    settings.chorus3_depth_ms =
        FixedPoint::from_float(chorus3_depth_ms.m_value);
    settings.chorus3_rate_steps =
        frequency_Hz_to_steps(chorus3_rate_Hz.m_value);
    settings.chorus3_delay_ms = chorus3_delay_ms.m_value;
    settings.chorus3_feedback =
        FixedPoint::from_float(chorus3_feedback.m_value);
    settings.tremolo_rate_steps =
        frequency_Hz_to_steps(tremolo_rate_Hz.m_value);
    settings.tremolo_depth = FixedPoint::from_float(tremolo_depth.m_value);
    settings.vibrato_depth_ms =
        FixedPoint::from_float(vibrato_depth_ms.m_value);
    settings.vibrato_rate_steps =
        frequency_Hz_to_steps(vibrato_rate_Hz.m_value);
  ...
  } else if (button_back.poll() || button_fwd.poll() || button_up.poll() ||
             button_down.poll()) {
    show_menu = true;
...
void guitar_effects_ui ::ui_2_patch(s_patch &patch) {
  ...
  changed |=
      assign_if_changed(patch.distortion_effect, distortion_effect.m_value);
  changed |=
      assign_if_changed(patch.distortion_offset, distortion_offset.m_value);
  ...
  changed |=
      assign_if_changed(patch.modulator_effect, modulator_effect.m_value);
  changed |=
      assign_if_changed(patch.flanger_depth_ms, flanger_depth_ms.m_value);
  changed |= assign_if_changed(patch.flanger_rate_Hz, flanger_rate_Hz.m_value);
  changed |=
      assign_if_changed(patch.flanger_delay_ms, flanger_delay_ms.m_value);
  changed |=
      assign_if_changed(patch.flanger_feedback, flanger_feedback.m_value);
  changed |=
      assign_if_changed(patch.chorus1_depth_ms, chorus1_depth_ms.m_value);
  changed |= assign_if_changed(patch.chorus1_rate_Hz, chorus1_rate_Hz.m_value);
  changed |=
      assign_if_changed(patch.chorus1_delay_ms, chorus1_delay_ms.m_value);
  changed |=
      assign_if_changed(patch.chorus1_feedback, chorus1_feedback.m_value);
  changed |=
      assign_if_changed(patch.chorus2_depth_ms, chorus2_depth_ms.m_value);
  changed |= assign_if_changed(patch.chorus2_rate_Hz, chorus2_rate_Hz.m_value);
  changed |=
      assign_if_changed(patch.chorus2_delay_ms, chorus2_delay_ms.m_value);
  changed |=
      assign_if_changed(patch.chorus2_feedback, chorus2_feedback.m_value);
  changed |=
      assign_if_changed(patch.chorus3_depth_ms, chorus3_depth_ms.m_value);
  changed |= assign_if_changed(patch.chorus3_rate_Hz, chorus3_rate_Hz.m_value);
  changed |=
      assign_if_changed(patch.chorus3_delay_ms, chorus3_delay_ms.m_value);
  changed |=
      assign_if_changed(patch.chorus3_feedback, chorus3_feedback.m_value);
  ...
  changed |=
      assign_if_changed(patch.vibrato_depth_ms, vibrato_depth_ms.m_value);
