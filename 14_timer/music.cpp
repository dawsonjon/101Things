...
void music::play_note(PWMAudio audio_output, note_t note) {
  ...
  const float amplitude =
      note.pitch_Hz == 0 ? 0 : 2047; // frequency of zero means silence
  ...
  while (samples_left) {
    ...
    for (uint16_t idx = 0; idx < block_size; idx++) {
      const float sample =
          sin_table[phase >> 22] * amplitude; // 10 MSBs (32-10 = 22)
...
void music::play_tune(PWMAudio audio_output, const note_t notes[],
                      uint16_t length) {
