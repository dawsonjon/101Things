...
void nco_start(double frequency_Hz)
{
    ...
    //this gives a maximim period of 256 clocks and minimum of 2, that 488.28125kHz to 62.5MHz
    ...
    //Each 256 bit waveform will not necassarily be a whole number of cycles, we will probably be part way through a cycle at the end.
    //This constant tells us how many clocks (and part clocks) we were through the last cycle at the end of a 256 bit waveform.
    //Next time, we chose a waveform that has been advanced by this many clocks so that we pick up where we left off (to the nearest clock).
    //The the fraction part is accumulated so that the any rounding error weintroduce can be corrected for later.
    //8 integer bits allow up to 256 whole clocks of advancement, the 24 remaining bit are fraction bits.
    ...
    //advancements of up to 31 bits are achieved by selecting the appriopriatly advanced waveform.
    ...
    buffer_addresses[0][33] = NULL; //null transfer at end of each 32 address block
    buffer_addresses[1][33] = NULL;
    ...
    for(uint8_t transfer = 0u; transfer < 32u; ++transfer)
    {
      ...
      uint8_t index = index_f24 >> fraction_bits;
    ...
    while(1) //Loop iterates at 125e6/(256*32) = 15259 Hz
    {
      ...
      //plan next 32 tranfers while last 32 are sending
      for(uint8_t transfer = 0u; transfer < 32u; ++transfer)
      {
        index_f24 += index_increment_f24;
        uint8_t index = index_f24 >> fraction_bits;
      ...
      //clocks of last one finishing don't put anything between here and next DMA
