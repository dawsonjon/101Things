...
int main() {
  ...
  // this gives a maximum period of 256 clocks and minimum of 2, that
  // 488.28125kHz to 62.5MHz
  ...
  // where we left off (to the nearest clock). The fraction part is accumulated
  // so that the any rounding error we introduce can be corrected for later. 8
  // integer bits allow up to 256 whole clocks of advancement, the 24 remaining
  // bits are fraction bits.
  const int32_t index_increment_f24 = round(
      fmod(waveform_length_bits, period_clocks) * pow(2.0, fraction_bits));

  // we may want to adjust the phase on-the fly.
  // An 8-bit phase adjustment will have steps of period/128 i.e. -period/128
  // +period/127
  const int32_t phase_step_clocks_f24 =
      (period_clocks / 128.0) * pow(2.0, fraction_bits);
