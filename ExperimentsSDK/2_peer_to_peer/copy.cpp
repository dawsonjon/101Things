...
class TCPServer
{
    ...
    uint8_t TCP2Application(char * buffer);

    uint8_t Application2TCP(char * buffer);
...
err_t server_sent_callback(void *arg, struct tcp_pcb *tpcb, u16_t len) {
    ...
    //The argument is the server object
...
err_t accept_callback(void *arg, struct tcp_pcb *pcb, err_t err) {
    ...
    if (err != ERR_OK || server->client_pcb == NULL) {
        DEBUG_printf("Failure in accept\n");
        return ERR_VAL;
    }
    DEBUG_printf("Client connected\n");

    server->client_pcb = pcb;
    tcp_arg(server->client_pcb, NULL);
    tcp_sent(server->client_pcb, server_sent_callback);
    tcp_recv(server->client_pcb, server_recv_callback);
    tcp_poll(server->client_pcb, server_poll_callback, POLL_TIME_S * 2);
    tcp_err(server->client_pcb, server_err_callback);
...
bool TCPServer::start()
{
    ...
    err_t err = tcp_bind(server_pcb, NULL, TCP_PORT);
    if (err) {
...
uint8_t TCPServer:: TCP2Application(char * buffer)
{
    printf(buffer);

}

uint8_t TCPServer:: Application2TCP(char * buffer)
{
