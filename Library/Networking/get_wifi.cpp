...
static int scan_result(void *env, const cyw43_ev_scan_result_t *result) 
{
  ...
  if (result) 
  {
      ...
      if(!match)
      {
        ...
        {
          strncpy(access_points->ssids[access_points->num_ssids++], (char*)result->ssid, 32);
        }
        else
        {
...
bool get_wifi()
{
  ...
  static const uint8_t __in_flash() __attribute__((aligned(FLASH_SECTOR_SIZE))) flash_sector[FLASH_SECTOR_SIZE] = {0xff};
  //The compiler "knows" that flash sector[0] contains 0xff, so replaces accesses to flash_sector[0] with that value.
  //To avoid this, cast to a volatile pointer, then the compiler actually reads from the appropriate location
  ...
  else
  {
    ...
    int err = cyw43_wifi_scan(&cyw43_state, &scan_options, &wifi_access_points, scan_result);
    if(err == 0)
    { 
    ...
    while(1)
    {
      ...
      if(sscanf(line, " %d", &idx) && idx < wifi_access_points.num_ssids && idx >= 0) break;
    }
