...
void menu :: draw()
{
  ...
  else ssd1306_draw_string(&disp, 0, 56, 1, centre("< RETURN >", 21));
...
bool menu :: poll()
{
  ...
  if(button_down.poll())
  {
    if(m_temp_value <= m_max) ++m_temp_value;
    redraw = true;
  }

  if(button_up.poll())
  {
    if(m_temp_value > 0) --m_temp_value;
    redraw = true;
  }
  ...
  {
    if(button_fwd.poll() || button_back.poll()) done = true; 
  }
